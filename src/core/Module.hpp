...
class Module {
  public:
    ...
    Module(const std::string& name, llvm::LLVMContext* context) : _llvm_context(context), llvm_module(new llvm::Module{name, *context}), _llvm_ir_builder{*context} {
        std::vector<llvm::Type*> main_param_types; //(1, llvm::Type::getInt32Ty(*_llvm_context));
        auto                     main_return_type = llvm::FunctionType::get(llvm::Type::getInt32Ty(*_llvm_context), main_param_types, false);
    ...
    inline llvm::Module&                 get_llvm_module() { return *llvm_module; }
    inline const llvm::Module&           get_llvm_module() const { return *llvm_module; }
    inline std::unique_ptr<llvm::Module> get_llvm_module_ptr() { return std::move(llvm_module); }
    ...
    llvm::AllocaInst* CreateEntryBlockAlloca(llvm::Function* func, auto type, const std::string& name) {
        if(func) {
        ...
        switch(node->type) {
            ...
            case AST::Node::Type::VariableDeclaration: {
                ...
                switch(node->value.type) {
                    case GenericValue::Type::Float: ret = CreateEntryBlockAlloca(parent_function, llvm::Type::getFloatTy(*_llvm_context), std::string{node->token.value}); break;
                    case GenericValue::Type::Integer: ret = CreateEntryBlockAlloca(parent_function, llvm::Type::getInt32Ty(*_llvm_context), std::string{node->token.value}); break;
